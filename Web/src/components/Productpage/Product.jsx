...
function Product() {
    ...
    useEffect(() => {
        ...
        if (!!recentProducts) {
            // console.log("recent Product Exists");
            let recentProductList = JSON.parse(recentProducts)
            let AlreadyExistance = false
            for (let index = 0; index < recentProductList.length; index++) {
                if (recentProductList[index] === pdt_id) {
            ...
            if (AlreadyExistance === false) {
                let recentProductItems = [pdt_id, ...recentProductList]
                if (recentProductList.length < 4) {
                    localStorage.setItem('recentProductsList', JSON.stringify(recentProductItems))
                } else {
                    localStorage.setItem('recentProductsList', JSON.stringify(recentProductItems.slice(0, 4)))
                }
            } else {
            ...
            let recentProducts = [pdt_id]
            localStorage.setItem('recentProductsList', JSON.stringify(recentProducts))
        }
    }, [])
                        ...
                        <div className="productImageContainer">
                            <div className="productImageoptions">
                                <div className="productimageOption" onClick={() => {
                                    ...
                                    <img src={pdtData.product_data.images[0]} alt="" />
                                </div>
                                <div className="productimageOption" onClick={() => {
                                    updateimage(2)
                                }}>
                                    <img src={pdtData.product_data.images[1]} alt="" />
                                </div>
                                <div className="productimageOption" onClick={() => {
                                    updateimage(3)
                                }}>
                                    <img src={pdtData.product_data.images[2]} alt="" />
                                </div>
                            </div>
                            <div className="productMainImage">
                                {(image === 2 || image === 1) ? ((image === 1) ? (<img src={pdtData.product_data.images[0]} alt="" />) : (<img src={pdtData.product_data.images[1]} alt="" />)) : (<img src={pdtData.product_data.images[2]} alt="" />)}
                            </div>
                        </div>
                        <div className="productInfo">
                            ...
                            <div className="pdtSpecification">
                                ...
                                <ul className="specificationlist">
                                    {
                                        Object.keys(pdtData.product_data.specifications).map((value, index) => {
                                            return (<li key={index}><span className="key">{value}</span><span className="value">{pdtData.product_data.specifications[value]}</span></li>
                                            )
                                        })
                                    }
                                </ul>
